...
func (q *Queries) ListRecentArticles(ctx context.Context, arg ListRecentArticlesParams) ([]ListRecentArticlesRow, error) {
	rows, err := q.db.QueryContext(ctx, listRecentArticles, arg.Published, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listRecentArticlesByFolder = `-- name: ListRecentArticlesByFolder :many
SELECT
    i.id,
    i.title,
    i.description,
...
type ListRecentArticlesByFolderParams struct {
	Published  sql.NullTime `json:"published"`
	FolderName string       `json:"folder_name"`
	Limit      int64        `json:"limit"`
}

type ListRecentArticlesByFolderRow struct {
	ID          int64        `json:"id"`
	Title       string       `json:"title"`
	Description string       `json:"description"`
	Link        string       `json:"link"`
...
func (q *Queries) ListRecentArticlesByFolder(ctx context.Context, arg ListRecentArticlesByFolderParams) ([]ListRecentArticlesByFolderRow, error) {
	rows, err := q.db.QueryContext(ctx, listRecentArticlesByFolder, arg.Published, arg.FolderName, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const listVisibleFeedsWithFolders = `-- name: ListVisibleFeedsWithFolders :many
SELECT
    f.id,
    f.title,
    f.url,
...
type ListVisibleFeedsWithFoldersRow struct {
	ID          int64  `json:"id"`
	Title       string `json:"title"`
	Url         string `json:"url"`
	TotalItems  int64  `json:"total_items"`
...
func (q *Queries) ListVisibleFeedsWithFolders(ctx context.Context) ([]ListVisibleFeedsWithFoldersRow, error) {
	rows, err := q.db.QueryContext(ctx, listVisibleFeedsWithFolders)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListVisibleFeedsWithFoldersRow
	for rows.Next() {
		var i ListVisibleFeedsWithFoldersRow
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.Url,
...
const searchArticlesSince = `-- name: SearchArticlesSince :many
SELECT
    i.id,
    i.title,
    i.description,
...
type SearchArticlesSinceParams struct {
	Published sql.NullTime   `json:"published"`
	Column2   sql.NullString `json:"column_2"`
...
type SearchArticlesSinceRow struct {
	ID          int64        `json:"id"`
	Title       string       `json:"title"`
	Description string       `json:"description"`
	Link        string       `json:"link"`
...
func (q *Queries) SearchArticlesSince(ctx context.Context, arg SearchArticlesSinceParams) ([]SearchArticlesSinceRow, error) {
	rows, err := q.db.QueryContext(ctx, searchArticlesSince,
		arg.Published,
		arg.Column2,
		arg.Column3,
		arg.Column4,
